...
wgcna_metadata_sym$changeRval <- c("-0.021575987", "-0.021575987", "-0.021575987", "-0.021575987", "-0.021575987", "-0.126644545", "-0.126644545", "-0.126644545", "-0.126644545", "-0.126644545", "0.175175245", "0.175175245", "0.175175245", "0.175175245", "0.175175245", "0.175175245",  "0.025518503", "0.025518503", "0.025518503", "0.025518503", "0.025518503")
...
softPower_sym=10 #smallest value to plateau at ~0.9 CHANGEING FROM 15 TO 10
adjacency_sym=adjacency(input_t_sym, power=softPower_sym,type="signed") #must change method type here too!!
```

```{r}
picked_power_sym = 10 ##smallest value to plateau at ~0.9 I pick 10 as it the the curve starts to plateu in my previous plot
temp_cor <- cor       
...
netwk_sym <- blockwiseModules(input_t_sym,        # <= input here
                          ...
                          # == Adjacency Function ==
                          power = picked_power_sym,          # <= power here
                          networkType = "signed",
...
mergedColors_sym = labels2colors(netwk_sym$colors)
# Plot the dendrogram and the module colors underneath
plotDendroAndColors(
  netwk_sym$dendrograms[[1]],
  mergedColors_sym[netwk_sym$blockGenes[[1]]],
  "Module colors",
  dendroLabels = FALSE,
  hang = 0.03,
  addGuide = TRUE,
  guideHang = 0.05 )
#dev.off()
# saved as dendrogram_thresh_sft10_signed_sym
```
...
TOM_sym<- TOMsimilarity(adjacency_sym,TOMType = "signed")##here
dissTOM_sym= 1-TOM_sym


geneTree_sym= flashClust(as.dist(dissTOM_sym), method="average")
plot(geneTree_sym, xlab="", sub="", main= "Gene Clustering on TOM-based dissimilarity", labels= FALSE,hang=0.04)
```
...
dynamicMods_sym= cutreeDynamic(dendro= geneTree_sym, distM= dissTOM_sym, deepSplit=2, pamRespectsDendro= FALSE, minClusterSize= minModuleSize)
table(dynamicMods_sym)


dynamicColors_sym= labels2colors(dynamicMods_sym)
#plot dendrogram and colors underneath
#sizeGrWindow(8,6)
plotDendroAndColors(geneTree_sym, dynamicColors_sym, "Dynamic Tree Cut", dendroLabels= FALSE, hang=0.03, addGuide= TRUE, guideHang= 0.05, main= "Gene dendrogram and module colors")
# saved as dendrogram_thresh_sft10_signed_2_sym

#Merg modules whose expression profiles are very similar
#calculate eigengenes
MEList_sym= moduleEigengenes(input_t_sym, colors= dynamicColors_sym)

MEs_sym= MEList_sym$eigengenes
...
#THE GREY MODULE IS WHERE WGCNA ASSIGNS GENES IT DOESN'T KNOW WHERE ELSE TO ASSIGN SO IT DOESN'T FOLLOW THE MIN SIZE AND ITS NOT A REAL NETWORK

MEs_sym = removeGreyME(MEs_sym, greyMEName = paste(moduleColor.getMEprefix(), "grey", sep=""))
#Calculate dissimilarity of module eigenegenes
MEDiss_sym= 1-cor(MEs_sym)
#Cluster module eigengenes
METree_sym= flashClust(as.dist(MEDiss_sym), method= "average")

#plot
plot(METree_sym, main= "Clustering of module eigengenes", xlab= "", sub= "")
MEDissThres_sym= 0.45 
abline(h=MEDissThres_sym, col="red")
#CLUSTER_TREE_Module_eigengenes_sym
```

```{r}
merge_sym= mergeCloseModules(input_t_sym, dynamicColors_sym, cutHeight= MEDissThres_sym, verbose =3)

mergedColors_sym= merge_sym$colors
mergedMEs_sym= merge_sym$newMEs

plotDendroAndColors(geneTree_sym, cbind(dynamicColors_sym, mergedColors_sym),
                    c("Dynamic Tree Cut", "Merged dynamic"),
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = FALSE, guideHang = 0.05,lwd=0.3)
#Cluster_dendo_MergeNetwork_dendrogram_mdist_45_sym
```

```{r}
# Rename to moduleColors
moduleColors_sym= mergedColors_sym
# Construct numerical labels corresponding to the colors
colorOrder_sym= c("grey", standardColors(50))
moduleLabels_sym= match(moduleColors_sym, colorOrder_sym)-1
MEs_sym=mergedMEs_sym
MEs_sym = removeGreyME(MEs_sym, greyMEName = paste(moduleColor.getMEprefix(), "grey", sep=""))
MEDiss_sym = 1-cor(MEs_sym);
# Cluster module eigengenes
METree_sym = flashClust(as.dist(MEDiss_sym), method = "average");
# Plot the result
plot(METree_sym, xlab = "Merged modules", sub = "")

#get number of genes in each module
table(moduleColors_sym)#here
```

```{r}
module_df_sym <- data.frame(
...
write_delim(module_df_sym,
            file = "./data_files/gene_modules_sym.txt",
            delim = "\t")

# Get Module Eigengenes per cluster
MEs0_sym <- moduleEigengenes(input_t_sym, mergedColors_sym)$eigengenes

# Reorder modules so similar modules are next to each other
MEs0_sym <- orderMEs(MEs0_sym)

#back into JK here
nGenes_sym <- ncol(input_t_sym) #16283
nSamples_sym <- nrow(input_t_sym) #21
# correlations of genes with eigengenes
moduleGeneCor_sym <- cor(MEs_sym, input_t_sym)
moduleGenePvalue_sym <- corPvalueStudent(moduleGeneCor_sym, nSamples_sym)
moduleTraitCor_sym <- cor(MEs_sym, wgcna_metadata_sym, use = "p")
moduleTraitPvalue_sym <- corPvalueStudent(moduleTraitCor_sym, nSamples_sym)

# module-trait correlations
textMatrix_sym <- paste(signif(moduleTraitCor_sym, 2), "\n(",
                    signif(moduleTraitPvalue_sym, 1), ")", sep = "");
dim(textMatrix_sym) <- dim(moduleTraitCor_sym)
par(mar = c(6, 8.5, 3, 3));
table(moduleColors_sym)
#moduleColors_sym
#     moduleColors_sym
#        black          blue         brown          cyan     darkgreen 
#         1659          1630          1549           403           232 
#      darkred darkturquoise         green   greenyellow     lightcyan 
#          256           568          1088           897           696 
#  lightyellow       magenta          pink           red     royalblue 
#          292           682          1337          2188           259 
#    turquoise 
#         2547
```
...
modLabels_sym <- sub("ME","",names(MEs_sym))
ps_sym <- signif(moduleTraitPvalue_sym,1)
cors_sym <- signif(moduleTraitCor_sym,2)
textMatrix_sym <-  cors_sym;
textMatrix_sym[ps_sym>0.05]="-"
dim(textMatrix_sym) <- dim(moduleTraitCor_sym)

mod_df_sym <- data.frame(module = row.names(moduleTraitCor_sym), moduleTraitCor_sym) %>% 
  gather("trait", "corr", -module, factor_key = TRUE) %>% 
  mutate(module = factor(module, levels = rownames(moduleTraitCor_sym))) %>% 
  mutate(module = fct_rev(module))

mod_pval_df_sym <- data.frame(module = row.names(ps_sym), ps_sym) %>% 
  gather("trait", "pval", -module, factor_key = TRUE) %>% 
  mutate(module = factor(module, levels = rownames(ps_sym))) %>% 
  mutate(module = fct_rev(module))

mod_df_sym <- mod_df_sym %>% 
  add_column(lab = mod_pval_df_sym$pval) %>% 
...
heatmap_sym <- ggplot(data = mod_df_sym, aes(x = trait, y = module, fill = corr)) +
  theme_bove() +
  geom_tile() +
  ...
  scale_x_discrete(labels = colnames(moduleTraitCor_sym)) +
...
heatmap_sym
#heatmap_prelim_sym_new

#### barplot of genes within each module
mct_sym <- table(moduleColors_sym)
modLabels_plot_sym <- as.data.frame(mct_sym[rev(modLabels_sym)])
modLabels_plot2_sym <- as.character(modLabels_plot_sym$moduleColors)
#modLabels2 <- gsub("turquoise", "teal", modLabels_plot2)

library(cowplot)
barplot_sym <- ggplot(data = modLabels_plot_sym, aes(y = Freq, x = moduleColors_sym, fill = moduleColors_sym)) +
  geom_col(colour = "black", size = 0.1) +
  scale_fill_manual(values = modLabels_plot2_sym) +
  theme_cowplot() +
  coord_flip(expand = FALSE) +
  #scale_y_manual(limits=c(8000, 0), breaks = c(1000, 2000, 3000,4000,5000,6000,7000,8000)) +
  geom_text(aes(label = paste0(rev(modLabels_sym), " (", Freq, ")")), hjust = 0.03, color="black", size = 3) +
  ...
  ylab("# genes\n per module")+
  scale_y_continuous(limits = c(0,3000), breaks = c(200, 400, 800, 1200, 1800, 2400, 3000))
barplot_sym

barplot2_sym <- ggplot(data = modLabels_plot_sym, aes(y = Freq, x = moduleColors_sym, fill = moduleColors_sym)) +
  geom_col(colour = "black", size = 0.1) +
  scale_fill_manual(values = modLabels_plot2_sym) +
  theme_cowplot() +
  coord_flip(expand = FALSE) +
  geom_text(aes(label = paste0(rev(modLabels_sym), " (", Freq, ")")), hjust = -0.1, color="black", size = 3) +  # Adjust hjust for better positioning
...
combined_plot_sym <- plot_grid(heatmap_sym, barplot2_sym, ncol = 1, align = "h", rel_widths = c(1, 1))
combined_plot_sym
#saved as wgcna_full_sym as 10x8 portrait
```
...
#        1659         3429         1501         1292         1150         2154 
#        grey       grey60    lightcyan midnightblue         pink       purple 
#          12          334          366          390          975          718 
#      salmon    turquoise 
#         564         2388 
```
...
# Combine heatmap and barplot side by side
combined_plot_apo <- plot_grid(heatmap_apo, barplot2_apo, ncol = 1, align = "h", rel_widths = c(1, 1))
combined_plot_apo
#saved as wgcna_full_apo as 12.5x5 no 8x8
```
...
modNames_sym <- substring(names(MEs_sym), 3) # string of all module names 

geneModuleMembership_sym <- as.data.frame(signedKME(input_t_sym,MEs_sym));
...
traitMods_sym <- sigMods(matrix = moduleTraitPvalue_sym,alpha=0.05) ## **NOTE: default alpha is set you 0.05 but can be adjusted by specifying 'alpha = XX' here
...
MEdarkgreen_sym <- read.table(file = "./data_files/Sym_Module_darkgreen_genes.txt")
MEdarkred_sym <- read.table(file = "./data_files/Sym_Module_darkred_genes.txt")
MEdarkturquoise_sym <- read.table(file = "./data_files/Sym_Module_darkturquoise_genes.txt")
...
MElightcyan_sym <- read.table(file = "./data_files/Sym_Module_lightcyan_genes.txt")
MElightyellow_sym <- read.table(file = "./data_files/Sym_Module_lightyellow_genes.txt")
MEmagenta_sym <- read.table(file = "./data_files/Sym_Module_magenta_genes.txt")
...
MEroyalblue_sym <- read.table(file = "./data_files/Sym_Module_royalblue_genes.txt")
MEturquoise_sym <- read.table(file = "./data_files/Sym_Module_turquoise_genes.txt")
```

```{r}
#change wd to data_files
setwd("./data_files/")
# Filter genes
...
module_files_sym <- list.files(pattern = "Sym_Module_.*_genes.txt")
...
for (file in module_files_sym) {
  ...
  output_file <- paste0("Sym_Presence_", module_name, ".csv")
...
remove quotes from the terms in the presence files
```{r}
#plot black module
...
#saved as pblack_symBP as portrait 20x10 --> THIS IS NEW IMMUNE
```
...
#MEblack_sym
general "immune": GO:0006955;GO:0045087;GO:0140546;GO:0098542|GO:0002376|	
GO:0002253;GO:0002757;GO:0002764|GO:0002684|GO:0050778|GO:0050776|GO:0002682|GO:0002252|GO:0002768|GO:0002429

nfkb: GO:0051092|GO:0043122

interleukin: GO:0032663

"defense": GO:0006952

random immune" GO:1903131;GO:0002521|GO:0050851|GO:0002335|GO:0050852|GO:0009615|GO:0030183|GO:1902105;GO:1902107;GO:1903708|GO:0045321;GO:0046649|GO:0030098|GO:0050864

cell death: GO:0010941;GO:0042981;GO:0043067|GO:0043066;GO:0043069;GO:0060548

cytokine: GO:0001817|GO:0034097;GO:0071345|GO:0019221
...
MEblacksym_name <- MEblack_sym
colnames(MEblacksym_name) <- paste(c("gene_ID"))

bp_black_sym = read.csv("./BP_Sym_Presence_black.csv", sep="\t")
head(bp_black_sym)
colnames(bp_black_sym)[4]="gene_ID"

go_bp_black_sym <- read.csv("./MWU_BP_Sym_Presence_black.csv", header=TRUE, sep=" ")
head(go_bp_black_sym)
write.table(go_bp_black_sym, "./MWU_BP_Sym_Presence_black.txt", quote=F, sep="\t", row.names=FALSE)
```

```{r}
bp_black_sym_imm <- bp_black_sym %>%
  filter(str_detect(term, "GO:0006955|GO:0045087|GO:0140546|GO:0098542|GO:0002376|GO:0002253|GO:0002757|GO:0002764|GO:0002684|GO:0050778|GO:0050776|GO:0002682|GO:0002252|GO:0002768|GO:0002429|GO:0051092|GO:0043122|GO:0032663|GO:0006952|GO:1903131|GO:0002521|GO:0050851|GO:0002335|GO:0050852|GO:0009615|GO:0030183|GO:1902105|GO:1902107|GO:1903708|GO:0045321|GO:0046649|GO:0030098|GO:0050864|GO:0010941|GO:0042981|GO:0043067|GO:0043066|GO:0043069|GO:0060548|GO:0001817|GO:0034097|GO:0071345|GO:0019221")) %>%
  distinct(gene_ID, .keep_all = TRUE) %>%
  select(gene_ID)
nrow(bp_black_sym_imm) #941
head(bp_black_sym_imm)
```
...
sig05_sym_P0C_name_black <- sig05_sym_P0C_name %>%
  inner_join(MEblacksym_name)
nrow(sig05_sym_P0C_name_black) #87

rld_degs_P0Cs_black <- rld_df_sym %>%
  rownames_to_column(var="gene_ID") %>%
  right_join(sig05_sym_P0C_name_black)
head(rld_degs_P0Cs_black)
nrow(rld_degs_P0Cs_black)
#87

immune_all_black_P0Cs = bp_black_sym %>%
  filter(str_detect(term, "GO:0006955|GO:0045087|GO:0140546|GO:0098542|GO:0002376|GO:0002253|GO:0002757|GO:0002764|GO:0002684|GO:0050778|GO:0050776|GO:0002682|GO:0002252|GO:0002768|GO:0002429|GO:0051092|GO:0043122|GO:0032663|GO:0006952|GO:1903131|GO:0002521|GO:0050851|GO:0002335|GO:0050852|GO:0009615|GO:0030183|GO:1902105|GO:1902107|GO:1903708|GO:0045321|GO:0046649|GO:0030098|GO:0050864|GO:0010941|GO:0042981|GO:0043067|GO:0043066|GO:0043069|GO:0060548|GO:0001817|GO:0034097|GO:0071345|GO:0019221")) %>% # select desired GO terms
  left_join(rld_degs_P0Cs_black) %>% 
  drop_na() %>%
  distinct(gene_ID, .keep_all = TRUE) %>%
  left_join(gene) %>%
...
head(immune_all_black_P0Cs)
nrow(immune_all_black_P0Cs)
#10

rld_means_immall_P0Cs_black = apply(immune_all_black_P0Cs, 1, mean)
explc_rld_immall_P0Cs_black = immune_all_black_P0Cs-rld_means_immall_P0Cs_black

imm_all_hm_P0Cs_black <- pheatmap(as.matrix(explc_rld_immall_P0Cs_black),
         ...
         scale="none",
         legend=T)
#saved as immune_all_hm_P0Csym_black 6x4 landscape
```
...
sig05_sym_P2C_name_black <- sig05_sym_P2C_name %>%
  inner_join(MEblacksym_name)
nrow(sig05_sym_P2C_name_black) #229

rld_degs_P2Cs_black <- rld_df_sym %>%
  rownames_to_column(var="gene_ID") %>%
  right_join(sig05_sym_P2C_name_black)
head(rld_degs_P2Cs_black)
nrow(rld_degs_P2Cs_black)
#229

immune_all_black_P2Cs = bp_black_sym %>%
  filter(str_detect(term, "GO:0006955|GO:0045087|GO:0140546|GO:0098542|GO:0002376|GO:0002253|GO:0002757|GO:0002764|GO:0002684|GO:0050778|GO:0050776|GO:0002682|GO:0002252|GO:0002768|GO:0002429|GO:0051092|GO:0043122|GO:0032663|GO:0006952|GO:1903131|GO:0002521|GO:0050851|GO:0002335|GO:0050852|GO:0009615|GO:0030183|GO:1902105|GO:1902107|GO:1903708|GO:0045321|GO:0046649|GO:0030098|GO:0050864|GO:0010941|GO:0042981|GO:0043067|GO:0043066|GO:0043069|GO:0060548|GO:0001817|GO:0034097|GO:0071345|GO:0019221")) %>% # select desired GO terms
  left_join(rld_degs_P2Cs_black) %>% 
  drop_na() %>%
  distinct(gene_ID, .keep_all = TRUE) %>%
  left_join(gene) %>%
  column_to_rownames(var = "gene_symbol") %>%
  dplyr::select(-name, -term, -lev, -value, -description, -gene_ID, -accession) %>%
  dplyr::select(sort(tidyselect::peek_vars()))
head(immune_all_black_P2Cs)
nrow(immune_all_black_P2Cs)
#30

rld_means_immall_P2Cs_black = apply(immune_all_black_P2Cs, 1, mean)
explc_rld_immall_P2Cs_black = immune_all_black_P2Cs-rld_means_immall_P2Cs_black

imm_all_hm_P2Cs_black <- pheatmap(as.matrix(explc_rld_immall_P2Cs_black),
         color=rev(colorRampPalette(brewer.pal(n=11, name="RdBu"))(20)),
         cluster_rows = T, show_rownames = T, fontsize_row=8, fontsize_col=8, treeheight_row = 8, treeheight_col = 10,
         cluster_cols = F, show_colnames = T, angle_col= "315", cellheight = 8, cellwidth = 11,
         gaps_col = c(10, 16),
         ...
         scale="none",
         legend=T)
#saved as immune_all_hm_P2Csym_black 6x5 landscape
```
...
sig05_sym_RC_name_black <- sig05_sym_RC_name %>%
  inner_join(MEblacksym_name)
nrow(sig05_sym_RC_name_black) #22

rld_degs_RCs_black <- rld_df_sym %>%
  rownames_to_column(var="gene_ID") %>%
  right_join(sig05_sym_RC_name_black)
head(rld_degs_RCs_black)
nrow(rld_degs_RCs_black)
#22

immune_all_black_RCs = bp_black_sym %>%
  filter(str_detect(term, "GO:0006955|GO:0045087|GO:0140546|GO:0098542|GO:0002376|GO:0002253|GO:0002757|GO:0002764|GO:0002684|GO:0050778|GO:0050776|GO:0002682|GO:0002252|GO:0002768|GO:0002429|GO:0051092|GO:0043122|GO:0032663|GO:0006952|GO:1903131|GO:0002521|GO:0050851|GO:0002335|GO:0050852|GO:0009615|GO:0030183|GO:1902105|GO:1902107|GO:1903708|GO:0045321|GO:0046649|GO:0030098|GO:0050864|GO:0010941|GO:0042981|GO:0043067|GO:0043066|GO:0043069|GO:0060548|GO:0001817|GO:0034097|GO:0071345|GO:0019221")) %>% # select desired GO terms
  left_join(rld_degs_RCs_black) %>% 
  drop_na() %>%
  distinct(gene_ID, .keep_all = TRUE) %>%
  left_join(gene) %>%
  column_to_rownames(var = "gene_symbol") %>%
  dplyr::select(-name, -term, -lev, -value, -description, -gene_ID, -accession) %>%
  dplyr::select(sort(tidyselect::peek_vars()))
head(immune_all_black_RCs)
nrow(immune_all_black_RCs)
#6

rld_means_immall_RCs_black = apply(immune_all_black_RCs, 1, mean)
explc_rld_immall_RCs_black = immune_all_black_RCs-rld_means_immall_RCs_black

imm_all_hm_RCs_black <- pheatmap(as.matrix(explc_rld_immall_RCs_black),
         color=rev(colorRampPalette(brewer.pal(n=11, name="RdBu"))(20)),
         cluster_rows = T, show_rownames = T, fontsize_row=8, fontsize_col=8, treeheight_row = 8, treeheight_col = 10,
         cluster_cols = F, show_colnames = T, angle_col= "315", cellheight = 8, cellwidth = 11,
         gaps_col = c(16),
         ...
         scale="none",
         legend=T)
#saved as immune_all_hm_RCsym_black 6x3 landscape
```


```{r}
#heatmap of all together
immune_all_black_ALLs <- rbind(immune_all_black_P0Cs, immune_all_black_P2Cs, immune_all_black_RCs) 
immune_all_black_ALLs <- unique(immune_all_black_ALLs)
nrow(immune_all_black_ALLs) #33

rld_means_immall_ALLs_black = apply(immune_all_black_ALLs, 1, mean)
explc_rld_immall_ALLs_black = immune_all_black_ALLs-rld_means_immall_ALLs_black

imm_all_hm_ALLs_black <- pheatmap(as.matrix(explc_rld_immall_ALLs_black),
         color=rev(colorRampPalette(brewer.pal(n=11, name="RdBu"))(20)),
         cluster_rows = T, show_rownames = T, fontsize_row=8, fontsize_col=8, treeheight_row = 8, treeheight_col = 10,
         ...
         scale="none",
         legend=T)
#saved as immune_all_hm_ALLsym_black 6x6 portrait
```
...
```{r}
bp_blue_apo_imm <- bp_blue_apo %>%
  filter(str_detect(term, "GO:0002376|GO:0006955;GO:0045087;GO:0140546;GO:0098542|GO:0002682|GO:0002761;GO:0002763;GO:0045639|GO:0010939|GO:0006952|GO:0097194|GO:0034612")) %>%
  distinct(gene_ID, .keep_all = TRUE) %>%
  select(gene_ID)
nrow(bp_blue_apo_imm) #565
head(bp_blue_apo_imm)

bp_blue_apo_tnf <- bp_blue_apo %>%
  filter(str_detect(term, "GO:0034612")) %>%
  distinct(gene_ID, .keep_all = TRUE) %>%
  select(gene_ID) %>%
  left_join(gene)
nrow(bp_blue_apo_tnf) #38
head(bp_blue_apo_tnf)
```
